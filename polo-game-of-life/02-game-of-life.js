...
document.querySelector("#myRange-speed").addEventListener('mouseup', function () {
    fr = parseInt(document.querySelector("#myRange-speed").value)
    speedValue.innerHTML = fr
});

document.querySelector("#new-life-rule").addEventListener('mouseup', function () {
    newLifeDueToReproduction = parseInt(document.querySelector("#new-life-rule").value)
    newLifeRuleText.innerHTML = newLifeDueToReproduction
});

document.querySelector("#rule-of-overpopulation").addEventListener('mouseup', function () {
    dieOfOverpopulation = parseInt(document.querySelector("#rule-of-overpopulation").value)
    dieOfOverpopulationText.innerHTML = dieOfOverpopulation
});

document.querySelector("#rule-of-loneliness").addEventListener('mouseup', function () {
    dieOfLoneliness = parseInt(document.querySelector("#rule-of-loneliness").value)
    dieOfLonelinessText.innerHTML = dieOfLoneliness
});

document.querySelector("#rule-of-box-sizing").addEventListener('click', function () {
    ...
    for (let x = 0; x < columns; x++) {
        for (let y = 0; y < rows; y++) {
            ...
            if (currentBoard[x][y] == 1 && neighbors < dieOfLoneliness) {
                ...
                nextBoard[x][y] = 0;
            } else if (currentBoard[x][y] == 0 && neighbors == newLifeDueToReproduction) {
...
document.querySelector('#clear-game')
    .addEventListener('click', function () {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j] = 0;
...
document.querySelector('#pattern-game1')
    .addEventListener('click', function () {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j] = 0;
...
document.querySelector('#pattern-game2')
    .addEventListener('click', function () {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j] = 0;
...
document.querySelector('#pattern-game3')
    .addEventListener('click', function () {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j] = 0;
...
document.querySelector('#pattern-game4')
    .addEventListener('click', function () {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j] = 0;
...
....xx......xx....`
        ...
        pattern.split('\n').map(line => line.split('').map(c => c == 'x' ? 1 : 0)).forEach((xs, y) => xs.forEach((value, x) => patternArr.push({
            x,
            y,
            value
        })))
        ...
        for (let i = 0; i < patternArr.length; i++) {
            currentBoard[patternArr[i]["x"] + x][patternArr[i]["y"] + y] = patternArr[i]["value"];
        }
        paint();
    });


document.querySelector('#pattern-game5')
    .addEventListener('click', function () {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j] = 0;
...
document.querySelector('#pattern-game6')
    .addEventListener('click', function () {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j] = 0;
        ...
        ${addDot(3)}${addX(2)}${addComma(1)}${addX(2)}${addComma(3)}${addX(1)}${addDot(3)}
        ${addDot(1)}${addX(2)}${addComma(4)}${addX(2)}${addComma(2)}${addX(1)}${addDot(3)}
        ...
        ${addDot(3)}${addX(2)}${addComma(4)}${addX(1)}${addComma(1)}${addX(2)}${addDot(3)}
        ${addDot(2)}${addX(1)}${addComma(1)}${addX(1)}${addComma(3)}${addX(1)}${addComma(3)}${addX(1)}${addDot(2)}
        ${addDot(2)}${addX(3)}${addComma(3)}${addX(4)}${addComma(1)}${addX(1)}${addDot(2)}
        ${addDot(4)}${addX(2)}${addComma(4)}${addX(1)}${addComma(2)}${addX(1)}${addDot(1)}
        ${addDot(2)}${addX(2)}${addComma(2)}${addX(4)}${addComma(3)}${addX(1)}${addDot(1)}
        ${addDot(1)}${addX(1)}${addComma(1)}${addX(1)}${addComma(1)}${addX(2)}${addDot(1)}${addX(1)}${addComma(1)}${addX(1)}${addX(1)}
        ${addDot(1)}${addX(6)}${addDot(1)}${addX(7)}`

        pattern.split('\n').map(line => line.split('').map(c => c == 'x' ? 3 : c == ',' ? 2 : 0)).forEach((xs, y) => xs.forEach((value, x) => patternArr.push({
            x,
            y,
            value
        })))
...
document.querySelector('#pattern-game7')
    .addEventListener('click', function () {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j] = 0;
...
${addDot(15)}${addX(1)}${addDot(3)}${addX(1)}${addDot(4)}${addX(2)}${addDot(12)}${addX(2)}
${addDot(4)}${addX(2)}${addDot(8)}${addX(1)}${addDot(5)}${addX(1)}${addDot(3)}${addX(2)}${addDot(14)}
${addDot(4)}${addX(2)}${addDot(8)}${addX(1)}${addDot(3)}${addX(1)}${addDot(1)}${addX(2)}${addDot(4)}${addX(1)}${addDot(1)}${addX(1)}${addDot(11)}
...
document.querySelector('#color-random')
    .addEventListener('click', function () {
        colorStyle = "random"
        paint()
    })
...
document.querySelector('#color-purple')
    .addEventListener('click', function () {
        colorStyle = "purple"
        paint()
    })

document.querySelector('#color-yellow')
    .addEventListener('click', function () {
        colorStyle = "yellow"
        paint()
    })
